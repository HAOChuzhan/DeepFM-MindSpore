...
class DeepFMModel(nn.Cell):
    ...
    def __init__(self, config):
        ...
        var_map = init_var_dict(self.init_args, init_acts) # init_args = [-0.01, 0.01]
        self.fm_w = var_map["W_l2"] # dense_ 数值特征
        self.embedding_table = var_map["V_l2"] # sparse_ 类别特征
        " Deep Layers "
        ...
        " FM, linear Layers "
        self.Gatherv2 = P.Gather()# 选取对应索引下axis的行或列
        self.Mul = P.Mul()
        self.ReduceSum = P.ReduceSum(keep_dims=False)
        self.Reshape = P.Reshape()
        self.Square = P.Square()  # 每个元素的平方
        self.Shape = P.Shape()    # 返回Tensor的shape
        self.Tile = P.Tile()      # 多次返回这个Tensor，组成新的tensor
        self.Concat = P.Concat(axis=1)
        self.Cast = P.Cast()      # 类型转换
    ...
    def construct(self, id_hldr, wt_hldr):
        ...
        fm_out = 0.5 * self.ReduceSum(v1 - v2, 1) # 和的平方减平方和即交叉特征之间的embedding点积
        fm_out = self.Reshape(fm_out, (-1, 1)) # (batch_size, 1)
        #  Deep layer
...
class NetWithLossClass(nn.Cell):
    ...
    def __init__(self, network, l2_coef=1e-6):
        ...
        self.loss = P.SigmoidCrossEntropyWithLogits() # 这里可以选择不同的损失函数
